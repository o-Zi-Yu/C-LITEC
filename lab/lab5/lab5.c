...
int main(void) {
    ...
    while (1) {
        ...
        desired_speed = desired_distance-measured_distance;
        desired_speed = desired_speed * kp;
        if (abs(desired_speed) < 10){
            desired_speed = 0;
        }
        if (abs(desired_speed) > 50){
            if (desired_speed>0){
                desired_speed = 50;
            }else{
                desired_speed = -50;
        ...
        if (desired_speed == 0){
            ...
            PWM_Calc();
            measured_speed1 = 1500000/PWM_Calc_avg1;
            speed_error1 = desired_speed - measured_speed1;
            error_sum1 += speed_error1;
            // printf("errorsum=%d\r\n",error_sum1);
            correct_speed1 = desired_speed + error_sum1;
            ...
            if(correct_speed1 < 10){
             ...
             printf("correct_speed: %d\r\n",correct_speed1);
            Timer_A_setCompareValue(TIMER_A0_BASE, TIMER_A_CAPTURECOMPARE_REGISTER_3, correct_speed_compare1);

            //Right wheel
            measured_speed2 = 1500000/PWM_Calc_avg2;
            speed_error2 = desired_speed - measured_speed2;
            error_sum2 += speed_error2;
            correct_speed2 = desired_speed + error_sum2;
            //correct_speed_compare = correct_speed*24000000/100;
